 ...
 *
 * @param paramName route parameter holding the child id (default `"childId"`).
 */
export function requireChildOwnership(paramName = "childId"): RequestHandler {
  return async (req: Request, _res: Response, next: NextFunction): Promise<void> => {
    try {
      if (!req.auth) return next(unauthorized());

      const rawParam = req.params[paramName];
      const childId = Array.isArray(rawParam) ? rawParam[0] : rawParam;
      if (!childId) return next(badRequest(`Missing route parameter: ${paramName}`));
